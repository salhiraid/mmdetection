...
from mmdet.models.detectors.rfdetr._rfdetr.assets.coco_classes import COCO_CLASS_NAMES, COCO_CLASSES
from mmdet.models.detectors.rfdetr._rfdetr.assets.model_weights import download_pretrain_weights, get_model_cache_dir
from mmdet.models.detectors.rfdetr._rfdetr.config import ModelConfig, TrainConfig
from mmdet.models.detectors.rfdetr._rfdetr.datasets._keypoint_schema import (
    active_keypoint_counts,
    infer_coco_keypoint_schema,
    infer_yolo_keypoint_schema,
)
from mmdet.models.detectors.rfdetr._rfdetr.datasets.coco import is_valid_coco_dataset
from mmdet.models.detectors.rfdetr._rfdetr.datasets.yolo import REQUIRED_YOLO_YAML_FILES, is_valid_yolo_dataset
from mmdet.models.detectors.rfdetr._rfdetr.inference import ModelContext, _build_model_context
from mmdet.models.detectors.rfdetr._rfdetr.utilities.distributed import is_main_process
from mmdet.models.detectors.rfdetr._rfdetr.utilities.keypoints import _is_bg_first_schema, precision_cholesky_to_pixel_covariance
...
class RFDETR:
    ...
    def from_checkpoint(cls, path: str | os.PathLike[str], *, trust_checkpoint: bool = False, **kwargs: Any) -> RFDETR:
        ...
        import mmdet.models.detectors.rfdetr._rfdetr.variants as rfdetr_variants

        _plus_available = False
        _plus_symbols: dict[str, type[RFDETR]] = {}
        _plus_entries: list[tuple[str, type[RFDETR]]] = []
        from mmdet.models.detectors.rfdetr._rfdetr.platform import _IS_RFDETR_PLUS_AVAILABLE

        if _IS_RFDETR_PLUS_AVAILABLE:
            try:
                import mmdet.models.detectors.rfdetr._rfdetr.platform.models as platform_models
        ...
        from mmdet.models.detectors.rfdetr._rfdetr.util.io import _safe_torch_load
        ...
        if model_cls is None:
            ...
            if not _plus_available and (plus_by_model_name or plus_by_weights_name):
                from mmdet.models.detectors.rfdetr._rfdetr.platform import _INSTALL_MSG
    ...
    def train(self, **kwargs):
        ...
        * ``notes`` — optional user-defined metadata (string, dict, list, or
        ...
        try:
            from mmdet.models.detectors.rfdetr._rfdetr.training import RFDETRDataModule, RFDETRModelModule, build_trainer
            from mmdet.models.detectors.rfdetr._rfdetr.training.auto_batch import resolve_auto_batch_config
            ...
            if exc.name and exc.name.startswith("mmdet.models.detectors.rfdetr._rfdetr."):
                raise
            raise ImportError(
        ...
        if config.save_dataset_grids and os.environ.get("LOCAL_RANK", "0") == "0":
            try:
                from mmdet.models.detectors.rfdetr._rfdetr.datasets.save_grids import DatasetGridSaver
    ...
    def export(
        ...
        *,
        ...
        try:
            from mmdet.models.detectors.rfdetr._rfdetr.export.main import export_onnx, make_infer_image
        except ImportError:
            logger.error(
        ...
        try:
            ...
            if format == "tflite":
                ...
                try:
                    from mmdet.models.detectors.rfdetr._rfdetr.export._tflite.converter import export_tflite
                except ImportError:
                    logger.error(
    ...
    def _align_keypoint_schema_from_dataset(self, config: TrainConfig) -> None:
        ...
        Example:
            >>> from mmdet.models.detectors.rfdetr._rfdetr.config import RFDETRKeypointPreviewConfig, TrainConfig
            >>> model = object.__new__(RFDETR)
...
def __getattr__(name: str):
    ...
    if name in _VARIANT_EXPORTS:
        module = importlib.import_module("mmdet.models.detectors.rfdetr._rfdetr.variants")
